...
const { default: worker } = await import(new URL(`../dist/server/index.js?check=${Date.now()}`, import.meta.url));
...
if (!login.ok) throw new Error(`No se pudo iniciar la prueba (${login.status}).`);
...
console.log(JSON.stringify({ ok: true, rows: result.rows.length }));
